...
def balance_frequency(name,allNames,prop):
	#need to solve this equation where x is the number of times we need to repeat the name 
	# allNames.count(name) + x
	# -------						= .25 
	# len(allNames) + x   

	return allNames + ([name] * int(round(  len(allNames) * (1/(1-prop)-1)  -  (1/(1-prop) * allNames.count(name)) )))
...
print allNames
allNames = balance_frequency(userVar['Name'],allNames,.25)
print allNames
